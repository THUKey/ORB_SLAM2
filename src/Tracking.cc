...
//for Sharing Map
//---------------------------------------------------------------------------------
bool Tracking::SharedFrameLocalization()
{
    // Compute Bag of Words Vector
    mCurrentFrame.ComputeBoW();
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        else
        {
            ...
            for(vector<MapPoint*>::const_iterator itMP=vvpMapPointMatches[i].begin(), itEndMP=vvpMapPointMatches[i].end(); itMP!=itEndMP; itMP++)
            {
                // itMP->SetWorldPosInSharedMap(mCurrentFrame.mWorldPos, mCurrentFrame.mnMachineId);
                MapPoint* pMP = *itMP;
                if(!pMP)
                    continue;
                #ifdef _DEBUG_
                //std::cout << "Find MapPoint:" << pMP->GetWorldPos() << std::endl;
                #endif
            }
    ...
    else
    {
        mnLastRelocFrameId = mCurrentFrame.mnId;
        #ifdef _DEBUG_
        std::cout << "now mCurrentFrame.mWorldPos is " << mCurrentFrame.mTcw << std::endl;
        //std::cout << "Show mCurrentFrame:  " << std::endl;
        //mCurrentFrame.Show();
        #endif
        return true;
    }
